...
#import "CBRealmObject.h"

@implementation CBRealmObject
...
- (instancetype)initWithDictionary:(NSDictionary *)dictionary error:(NSError **)error {
    ...
    for (NSString *key in dictionary) {
        ...
        BOOL success = YES;//MTLValidateAndSetValue(self, key, value, YES, error);
...
+ (NSSet *)propertyKeys {
    NSSet *cachedKeys = objc_getAssociatedObject(self, "");
    ...
    // It doesn't really matter if we replace another thread's work, since we do
    // it atomically and the result should be the same.
    objc_setAssociatedObject(self, "", keys, OBJC_ASSOCIATION_COPY);
    
    return keys;
}

+ (NSSet *)transitoryPropertyKeys {
    NSSet *transitoryPropertyKeys = objc_getAssociatedObject(self, "");
    
    if (transitoryPropertyKeys == nil) {
        [self generateAndCacheStorageBehaviors];
        transitoryPropertyKeys = objc_getAssociatedObject(self, "");
    }
...
+ (NSSet *)permanentPropertyKeys {
    NSSet *permanentPropertyKeys = objc_getAssociatedObject(self, "");
    
    if (permanentPropertyKeys == nil) {
        [self generateAndCacheStorageBehaviors];
        permanentPropertyKeys = objc_getAssociatedObject(self, "");
    }
...
- (void)mergeValueForKey:(NSString *)key fromModel:(NSObject<MTLModel> *)model {
    ...
    SEL selector = nil;// MTLSelectorWithCapitalizedKeyPattern("merge", key, "FromModel:");
...
- (BOOL)validate:(NSError **)error {
    for (NSString *key in self.class.propertyKeys) {
        ...
        BOOL success = YES;//MTLValidateAndSetValue(self, key, value, NO, error);
